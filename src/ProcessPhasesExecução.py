...
from ProcessCardExecução import process_card_execução
...
def process_phases_execução(ws, headers, all_phases, row_num):
    clear_worksheet(ws)
    ...
    for phase in all_phases:
        if isinstance(phase, dict):
            for card_edge in phase['cards']['edges']:
                card = card_edge['node']
                field_values = process_card_execução(card, headers)
